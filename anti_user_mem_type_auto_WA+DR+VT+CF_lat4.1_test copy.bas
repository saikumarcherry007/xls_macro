...
Private Sub ParseWAMemoryType(ByVal colName As String, ByRef baseName As String, ByRef waValues As Collection, ByRef hasWA As Boolean)
    Dim pStart As Long
    Dim pEnd As Long
    Dim waContent As String
    Dim parts As Variant
    Dim p As Variant
    Dim cleanColName As String
    
    Set waValues = New Collection
    hasWA = False
    ...
    ' Look for (WA=
    pStart = InStr(1, colName, "(WA=", vbTextCompare)
    If pStart > 0 Then
        ' Extract base name (everything before (WA=)
        baseName = Left(colName, pStart - 1)
        
        ' Clean up base name (remove newlines if present)
        If InStr(baseName, vbLf) > 0 Then baseName = Replace(baseName, vbLf, "")
        If InStr(baseName, vbCr) > 0 Then baseName = Replace(baseName, vbCr, "")
        baseName = Trim(baseName)
        
        ' Extract WA content
        pEnd = InStr(pStart, colName, ")")
        If pEnd > 0 Then
            waContent = Mid(colName, pStart + 4, pEnd - (pStart + 4)) ' Length of "(WA=" is 4
            parts = Split(waContent, "/")
            For Each p In parts
                waValues.Add Trim(CStr(p))
            Next p
            hasWA = True
        End If
    Else
        ...
        ' Handle other parentheses if any (legacy logic)
        pStart = InStr(baseName, "(")
        If pStart > 0 Then baseName = Left(baseName, pStart - 1)
...
Private Function GetUniqueDRValuesFromInstanceList(ByRef outMsg As String) As String
    ...
    Dim patternToValues As Object
    Set patternToValues = CreateObject("Scripting.Dictionary")
    ...
    For Each key In drValues.Keys
        ...
        If patMatches Then
            ...
            ' Accumulate found values for this pattern
            If Not patternToValues.Exists(UCase(wildcardPat)) Then
                patternToValues.Add UCase(wildcardPat), originalVal
            Else
                ' Check if value is already in list (simple check to avoid dupes if key iteration repeats somehow, though unlikely with Keys)
                If InStr(1, patternToValues(UCase(wildcardPat)), originalVal, vbTextCompare) = 0 Then
                     patternToValues(UCase(wildcardPat)) = patternToValues(UCase(wildcardPat)) & ", " & originalVal
                End If
            End If
        ...
        Else
            ...
            If Not finalFilters.Exists(UCase(originalVal)) Then
                ...
                ' Track value for message
                patternToValues.Add UCase(originalVal), originalVal
            End If
        End If
    Next key
    
    ' Step 3: Build output string and message
    foundList = ""
    Dim fKey As Variant
    For Each fKey In finalFilters.Keys
        Dim fVal As String
        fVal = finalFilters(fKey)
        Dim valList As String
        valList = ""
        If patternToValues.Exists(UCase(fVal)) Then valList = patternToValues(UCase(fVal))
        
        If foundList = "" Then
            foundList = fVal & " (found for " & valList & ")"
        Else
            foundList = foundList & vbCrLf & "  • " & fVal & " (found for " & valList & ")"
        End If
    Next fKey
    
    ' Step 4: Build comma-separated string for return
    drList = ""
    For Each fKey In finalFilters.Keys
        If drList = "" Then
            drList = finalFilters(fKey)
        Else
            drList = drList & "," & finalFilters(fKey)
        End If
    Next fKey
...
Private Function GetUniqueMemoryTypesFromInstanceList(ByRef outMsg As String, ByRef unmatchedMsg As String) As String
    ...
    For Each key In memTypes.Keys
        ...
        If pvtHeaders.Count > 0 Then
            Dim hdr As Variant
            Dim baseHdr As String
            Dim waVals As Collection
            Dim hasWAStr As Boolean
            Dim matchedHdr As String
            matchedHdr = ""
            
            For Each hdr In pvtHeaders
                ' Parse the PVT header to see if it matches our basic memory type
                ParseWAMemoryType CStr(hdr), baseHdr, waVals, hasWAStr
                
                ' Check if base name matches (exact match)
                If UCase(Trim(baseHdr)) = UCase(originalMemType) Then
                    found = True
                    matchedHdr = CStr(hdr)
                    Exit For
                End If
        ...
        If found Then
            ' Use the ACTUAL header from PVT sheet (hdr key is used in loop above, but we need to retrieve it)
            ' Re-find the matching header to be safe or use simple logic
            ' Since we broke on match, 'hdr' variable holds the matching header?
            ' Wait, 'hdr' loop variable scope might be tricky if not captured.
            ' Let's capture matchedHdr inside the loop.
            
            ' Note: Logic above needs to capture 'hdr' into a variable 'matchedHdr'
            If Not matchedTypes.Exists(UCase(matchedHdr)) Then
                matchedTypes.Add UCase(matchedHdr), matchedHdr
                If foundList = "" Then
                    foundList = matchedHdr
                Else
                    foundList = foundList & vbCrLf & "  • " & matchedHdr
                End If
            End If
        Else
    ...
    For Each inst In allInstances
        ...
        If autoFilterActive Then
            ...
            If inst.Exists("MEMORY_TYPE") Then
                ...
                For Each colName In autoMemCols
                    If UCase(pvtRowData(colName)) = "YES" Then
                        ' Parse column name for potential WA constraint
                        Dim baseMemName As String
                        Dim waConstraints As Collection
                        Dim hasWAFilter As Boolean
                        
                        ParseWAMemoryType CStr(colName), baseMemName, waConstraints, hasWAFilter
                        
                        ' Check if instance memory type matches this column name's base type
                        ' (Simple wildcard match or exact match depending on logic)
                        If instVal = UCase(baseMemName) Or MatchesWildcard(instVal, UCase(baseMemName) & "*") Then
                            
                            Dim waMatch As Boolean
                            waMatch = True
                            
                            ' If WA constraint exists, apply it
                            If hasWAFilter Then
                                waMatch = False
                                If inst.Exists("WRITE_ASSIST") Then
                                    Dim instWA As String
                                    instWA = Trim(CStr(inst("WRITE_ASSIST")))
                                    
                                    ' Check if instance WA is in allowed values
                                    Dim wav As Variant
                                    For Each wav In waConstraints
                                        If UCase(instWA) = UCase(CStr(wav)) Then
                                            waMatch = True
                                            Exit For
                                        End If
                                    Next wav
                                Else
                                    ' WA column missing in instance but required by filter -> fail
                                    waMatch = False
                                End If
                            End If
